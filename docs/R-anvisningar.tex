...
\begin{center}\includegraphics{R-anvisningar_files/figure-latex/unnamed-chunk-59-1} \end{center}
...
Om man vill skapa en kolumn med mellanrum i namnet måste man skriva namnet inom \emph{backticks} \texttt{\textbackslash{}\textasciigrave{}\textasciigrave{}\ för\ att\ ange\ att\ namnet\ ska\ tolkas\ som\ en\ enhet.\ Jag\ rekommenderar\ att\ undvika\ mellanrum\ i\ kolumnnamn\ och\ istället\ använda\ stora\ bokstäver\ eller\ understreck\ för\ ett\ nytt\ ord\ (}NationalGDP\texttt{eller}National\_GDP`).
...
\begin{center}\includegraphics{R-anvisningar_files/figure-latex/unnamed-chunk-89-1} \end{center}
...
En diskret slumpvariabel kan bara anta specifika värden på tallinjen. Det absolut vanligaste fallet är när utfallen är heltal. Varje utfall är kopplat till \emph{en} sannolikhet och summan av sannolikheterna är ett.
...
\begin{center}\includegraphics{R-anvisningar_files/figure-latex/unnamed-chunk-116-1} \end{center}

\begin{exercise}[En annan tärning]
Ta fram en tärning som \emph{inte} har sex sidor. Följ exemplet ovan för att kodifiera din tärning som en slumpvariabel. Ett bra namn på det nya objektet kan vara \texttt{dat\_diceN} där N anger antalet sidor på tärningen, t.ex. \texttt{dat\_dice20} för en 20-sidig tärning.
\end{exercise}
...
För en diskret slumpvariabel ges medelvärdet (som då också kallas \emph{populationsmedelvärde}, eller vanligare \emph{väntevärde}) av summan av utfallen gånger sannolikheterna. För vår tärning kan vi räkna ut det genom att multiplicera utfall och sannolikhet i ett \texttt{mutate()} steg och sedan summera.
...
Beräkningen av en teoretisk varians (som vi kan kalla \emph{populationsvarians}) är lite mer komplicerad och vi går inte in på några detaljer här. Koden nedan skapar en funktion som beräknar populationsvariansen.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pop\_var }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(x, p) }\FunctionTok{sum}\NormalTok{(p }\SpecialCharTok{*}\NormalTok{ (x }\SpecialCharTok{{-}} \FunctionTok{sum}\NormalTok{(x }\SpecialCharTok{*}\NormalTok{ p))}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{dat\_dice6 }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{summarise}\NormalTok{(}\AttributeTok{varians =} \FunctionTok{pop\_var}\NormalTok{(x, p))}
\end{Highlighting}
\end{Shaded}
...
R kommer också med en rad funktioner för att simulera data. Funktionen \texttt{sample()} drar ett urval ur en serie möjliga utfall. Följande kod drar tiotusen slumputfall från en 6-sidig tärning, och beräknar medelvärde och varians.
...
Som exempel kan vi ta ett tärningskast. Om man kastar en 6-sidig tärning tio gånger kommer antalet ettor (vårt positiva utfall) följa en binomialfördelning där n ges av tio och p av en sjättedel. I R kan sannolikheter från en binomialfördelning tas fram med \texttt{dbinom()}. Sannolikheten för tre ettor ges till exempel av

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{dbinom}\NormalTok{(}\DecValTok{3}\NormalTok{, }\AttributeTok{size =} \DecValTok{10}\NormalTok{, }\AttributeTok{prob =} \DecValTok{1}\SpecialCharTok{/}\DecValTok{6}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{dat\_bin }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{x =} \DecValTok{0}\SpecialCharTok{:}\DecValTok{10}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{p =} \FunctionTok{dbinom}\NormalTok{(x, }\AttributeTok{size =} \DecValTok{10}\NormalTok{, }\AttributeTok{prob =} \DecValTok{1}\SpecialCharTok{/}\DecValTok{6}\NormalTok{))}
...
I den fördelning du beräknade i uppgiften ovan. Vad är sannolikheten att få exakt tre positiva utfall? (Ledning: för en sexsidig tärning skulle det ges av \texttt{dbinom(3,\ size\ =\ 20,\ prob\ =\ 1/6))}.)
\end{exercise}
...
I R beräknas fördelningsfunktionen med \texttt{pbinom()}. För fallet med tio tärningskast kan man ta

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dat\_bin }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{x =} \DecValTok{0}\SpecialCharTok{:}\DecValTok{10}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{p =} \FunctionTok{dbinom}\NormalTok{(x, }\AttributeTok{size =} \DecValTok{10}\NormalTok{, }\AttributeTok{prob =} \DecValTok{1}\SpecialCharTok{/}\DecValTok{6}\NormalTok{),}
         \AttributeTok{P =} \FunctionTok{pbinom}\NormalTok{(x, }\AttributeTok{size =} \DecValTok{10}\NormalTok{, }\AttributeTok{prob =} \DecValTok{1}\SpecialCharTok{/}\DecValTok{6}\NormalTok{))}
\NormalTok{dat\_bin}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{pbinom}\NormalTok{(}\DecValTok{4}\NormalTok{, }\AttributeTok{size =} \DecValTok{10}\NormalTok{, }\AttributeTok{prob =} \DecValTok{1}\SpecialCharTok{/}\DecValTok{6}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{ggplot}\NormalTok{(dat\_bin, }\FunctionTok{aes}\NormalTok{(x, p, }\AttributeTok{fill =}\NormalTok{ x }\SpecialCharTok{\textless{}=} \DecValTok{4}\NormalTok{)) }\SpecialCharTok{+}
  \FunctionTok{geom\_col}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ppois}\NormalTok{(}\DecValTok{2}\NormalTok{, }\AttributeTok{lambda =} \DecValTok{4}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{center}\includegraphics{R-anvisningar_files/figure-latex/unnamed-chunk-129-1} \end{center}
...
Beräkna först sannolikheten för hand och sedan med \texttt{pnorm()}.
...
\begin{center}\includegraphics{R-anvisningar_files/figure-latex/unnamed-chunk-152-1} \end{center}
...
\begin{center}\includegraphics{R-anvisningar_files/figure-latex/unnamed-chunk-152-2} \end{center}
...
\begin{center}\includegraphics{R-anvisningar_files/figure-latex/unnamed-chunk-153-1} \end{center}
...
\begin{center}\includegraphics{R-anvisningar_files/figure-latex/unnamed-chunk-164-1} \end{center}
...
\begin{center}\includegraphics{R-anvisningar_files/figure-latex/unnamed-chunk-174-1} \end{center}
...
\begin{exercise}[Herrallsvenskan]
Följande importerar data för herrallsvenskan i fotboll.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dat }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}\StringTok{"https://raw.githubusercontent.com/adamflr/ST0060{-}2022/main/Data/Allsvenskan\%2C\%20herrar\%2C\%201924{-}2020.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{exercise}[Antal hemmasegrar]
Beräkna antalet hemmasegrar i herrallsvenskan genom att räkna antalet matcher där \texttt{Hemmamål} överstiger \texttt{Bortamål}.
\end{exercise}

\begin{exercise}[Test för antal hemmasegrar]
Testa om andelen hemmasegrar skiljer sig från nollhypotesen på 47 procent.
\end{exercise}

\begin{exercise}[Konfidensintervall för antal hemmasegrar]
Använd \texttt{binom.asymp()} från paketet \texttt{binom} för att beräkna ett konfidensintervall för andelen hemmasegrar. Jämför med en handräkning (hand och miniräknare).
\end{exercise}

\begin{exercise}[Hemmasegrar för Malmö FF]

Följande beräknar antalet hemmasegrar för Malmö FF. Testa om Malmös andel hemmasegrar är större än 47 procent.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dat }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{filter}\NormalTok{(Hemmalag }\SpecialCharTok{==} \StringTok{"Malmö FF"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{count}\NormalTok{(Hemmamål }\SpecialCharTok{\textgreater{}}\NormalTok{ Bortamål)}
\end{Highlighting}
\end{Shaded}
...
\begin{center}\includegraphics{R-anvisningar_files/figure-latex/unnamed-chunk-274-1} \end{center}
...
\begin{center}\includegraphics{R-anvisningar_files/figure-latex/unnamed-chunk-275-1} \end{center}
...
\begin{center}\includegraphics{R-anvisningar_files/figure-latex/unnamed-chunk-281-1} \end{center}
...
## $emmeans
##  group  emmean    SE df lower.CL upper.CL
##  a     -0.3576 0.331 45   -1.025    0.310
##  b      0.0628 0.331 45   -0.605    0.730
##  c      0.1539 0.331 45   -0.514    0.821
##  d     -0.0534 0.331 45   -0.721    0.614
##  e      0.1263 0.331 45   -0.541    0.794
## 
## Confidence level used: 0.95 
## 
## $contrasts
##  contrast estimate    SE df t.ratio p.value
##  a - b     -0.4204 0.469 45  -0.897  0.3745
##  a - c     -0.5115 0.469 45  -1.091  0.2810
##  a - d     -0.3042 0.469 45  -0.649  0.5197
##  a - e     -0.4839 0.469 45  -1.032  0.3074
##  b - c     -0.0911 0.469 45  -0.194  0.8469
##  b - d      0.1163 0.469 45   0.248  0.8052
##  b - e     -0.0635 0.469 45  -0.135  0.8929
##  c - d      0.2073 0.469 45   0.442  0.6604
##  c - e      0.0276 0.469 45   0.059  0.9533
##  d - e     -0.1797 0.469 45  -0.383  0.7032
\end{verbatim}
...
\begin{center}\includegraphics{R-anvisningar_files/figure-latex/unnamed-chunk-306-1} \end{center}
...
\begin{center}\includegraphics{R-anvisningar_files/figure-latex/unnamed-chunk-313-1} \end{center}
...
\begin{center}\includegraphics{R-anvisningar_files/figure-latex/unnamed-chunk-314-1} \end{center}
...
\begin{center}\includegraphics{R-anvisningar_files/figure-latex/unnamed-chunk-332-2} \end{center}
