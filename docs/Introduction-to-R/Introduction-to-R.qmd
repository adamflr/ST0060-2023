...
<div style="position:absolute;width:30%; left:33%;top:0;">
...
<div style="position:absolute;width:30%; left:67%;top:0;">
3\. Use piping (`%>%` or `|>`) to send output into the next function

```{r}
c(3, 14, 159) %>% # Take the data, then...
  log() %>% # ... log-transform, then...
  sum() # ... sum
```
...
```{r}
dat <- data.frame(a, b)
as_tibble(dat) # Using a tibble to get a display of the mode
```
...
```{r}
dat$a <- as.character(dat$a) # Changes the first column to character
as_tibble(dat)
```

The mode can also be specified in most import functions
